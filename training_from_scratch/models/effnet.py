...
def build_effnet(inputs, blocks_per_subnet=(1, 1, 1), num_classes=10, channels_per_subnet=(32, 64, 128, 256),
                 expansion_rate=2):
    ...
    for i, nb_blocks in enumerate(blocks_per_subnet):
        ...
        for _ in range(i-1):
            x = effnet_block(x, ch_bottle, ch_bottle, stride=False)
        x = effnet_block(x, ch_bottle, ch_out, stride=True)
...
def effnet_block(x_in, ch_bottle, ch_out, stride):
    ...
    x = Conv2D(ch_bottle, kernel_size=1, padding="same", use_bias=False)(x_in)
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)
    ...
    if stride:
        x = Conv2D(ch_out, kernel_size=(2, 1), strides=(2, 1), padding="same", use_bias=False)(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
